...
public class ScrollFeedbackManager implements AccessibilityEventListener {
  ...
  /** Event types that are handled by ScrollPositionInterpreter. */
  ...
  private ScrollPositionHandler getHandler() {
    // Handler requires an initialized this-argument, so create handler on-demand.
    if (handler == null) {
      handler = new ScrollPositionHandler(this);
    }
    return handler;
  }
  ...
  private @Nullable CharSequence getDescriptionForScrollEvent(AccessibilityEvent event) {
  ...
  private static @Nullable CharSequence getSelectedPageTitle(AccessibilityNodeInfo node) {
  ...
  private static class EventId {
    ...
    public EventId(long nodeId, int windowId) {
      ...
      this.hashcode = toHashcode(nodeId, windowId);
    }

    private static int toHashcode(long nodeId, int windowId) {
      return (int) (nodeId ^ (nodeId >>> 32)) + windowId * 7;
    }

    public EventId(AccessibilityEvent event)
        throws InvocationTargetException, IllegalAccessException {
      @Nullable Object nodeId = getSourceNodeIdMethod.invoke(event);
      this.nodeId = (nodeId == null) ? 0 : (long) nodeId;
      this.windowId = event.getWindowId();
      this.hashcode = toHashcode(this.nodeId, this.windowId);
    }
