 ...
 */
public class TextToSpeechSimpleOverlay extends SimpleOverlay implements TextToSpeechOverlay {
  ...
  public TextToSpeechSimpleOverlay(Context context) {
    this(context, /* id= */ 0, /* sendsAccessibilityEvents= */ false);
  }

  public TextToSpeechSimpleOverlay(
      Context context, int id, final boolean sendsAccessibilityEvents) {
  ...
  private static class OverlayHandler extends WeakReferenceHandler<TextToSpeechSimpleOverlay> {
    public OverlayHandler(TextToSpeechSimpleOverlay parent) {
      super(parent);
    }

    @Override
    protected void handleMessage(Message msg, TextToSpeechSimpleOverlay parent) {
      switch (msg.what) {
        case MSG_SET_TEXT -> {
          try {
            parent.show();
