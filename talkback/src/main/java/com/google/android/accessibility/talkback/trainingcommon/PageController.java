...
import static com.google.android.accessibility.talkback.analytics.TalkBackAnalytics.TRAINING_BUTTON_NEXT;
import static com.google.android.accessibility.talkback.analytics.TalkBackAnalytics.TRAINING_BUTTON_PREVIOUS;
...
public class PageController {
  ...
  private Function<Context, NavigationButtonBar> createNavigationBarSupplier() {
    ...
    boolean isFirstOrOnePage = isFirstPage() || isOnePage();
    PageConfig targetPage = training.getPages().get(currentPageNumber);
    ImmutableList<Integer> navigationButtons =
        targetPage.getNavigationButtons() == null
            ? training.getButtons()
            : targetPage.getNavigationButtons();
  ...
  public boolean previousPage() {
    ...
    if (metricStore != null) {
      metricStore.onTutorialEvent(TRAINING_BUTTON_PREVIOUS);
    }
    return true;
  }
  ...
  private int findPageNumberByName(@StringRes int pageNameResId) {
    ...
    for (int i = 0; i < pages.size(); i++) {
      if (pages.get(i).getPageNameResId() == pageNameResId) {
        return i;
      }
    }
    return UNKNOWN_PAGE_NUMBER;
