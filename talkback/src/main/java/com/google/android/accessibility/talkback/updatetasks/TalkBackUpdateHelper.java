...
import static com.google.android.accessibility.talkback.TalkBackService.PREF_HAS_ONBOARDING_FINISHED;
...
public class TalkBackUpdateHelper {
  ...
  public void checkUpdate() {
    ...
    String previousVersionName =
        sharedPreferences.getString(PREF_APP_PREV_VERSION_NAME, /* defValue= */ null);
    ...
    try {
      packageInfo = pm.getPackageInfo(service.getPackageName(), 0);
    ...
    if (FormFactorUtils.isAndroidWear()) {
      ...
      if ((previousVersion != VERSION_CODE_UNKNOWN) && (previousVersion < 60121543)) {
      ...
      if ((previousVersion != VERSION_CODE_UNKNOWN) && (previousVersion < 60123270)) {
        editor.remove(service.getString(R.string.pref_log_level_key));
        PreferencesActivityUtils.removeEditingKey(service.getApplicationContext());
      }
    }
    ...
    if (previousVersion != VERSION_CODE_UNKNOWN) {
      editor
          .putBoolean(PREF_HAS_TRAINING_FINISHED, true)
          .remove(PREF_HAS_ONBOARDING_FINISHED)
          .apply();
      ...
      // Disabled show water settings for genuine update users.
      if ((SharedPreferencesUtils.getFloatFromStringPref(
                  sharedPreferences,
                  service.getResources(),
                  R.string.pref_speech_rate_key,
                  R.string.pref_speech_rate_default)
              > Float.parseFloat(service.getString(R.string.pref_speech_rate_default))
          || sharedPreferences.getBoolean(
              service.getString(R.string.has_training_page_switched), false))) {
        sharedPreferences
            .edit()
            .putBoolean(service.getString(R.string.pref_show_exit_watermark_key), false)
            .apply();
      }
    }
    ...
    TalkBackUpdateManager talkBackUpdateManager = new TalkBackUpdateManager();
    talkBackUpdateManager.startMigration(packageInfo.versionName, previousVersionName);
