...
public class VolumeMonitor extends SameThreadBroadcastReceiver {
  ...
  @VisibleForTesting
  static final Duration RELEASE_CONTROL_TIMEOUT_MS_DEFAULT = Duration.ofMillis(2000);

  @VisibleForTesting static final Duration RELEASE_CONTROL_TIMEOUT_MS_WEAR = Duration.ofMillis(600);

  /** An interface for listening the changed volume. */
  public interface VolumeChangedListener {
    /** Invoked when the volume has been changed. */
    void onVolumeChanged(int type, int value, int prevValue, int percent);
  }
  ...
  private final List<VolumeChangedListener> listeners = new ArrayList<>();

  /** Creates and initializes a new volume monitor. */
  public VolumeMonitor(
      FeedbackReturner pipeline, Context context, CallStateMonitor callStateMonitor) {
    if (pipeline == null) {
  ...
  public void clearVolumeChangedListener() {
    listeners.clear();
  }

  private void notifyVolumeChangedListener(int type, int value, int prevValue, int percent) {
    for (VolumeChangedListener listener : listeners) {
      listener.onVolumeChanged(type, value, prevValue, percent);
    }
  }
  ...
  private void internalOnVolumeChanged(int streamType, int volume, int prevVolume) {
    ...
    notifyVolumeChangedListener(streamType, volume, prevVolume, getStreamVolume(streamType));
  ...
  private void internalSpeakA11yVolumeControlHint() {
    ...
    Part.Builder part = Feedback.Part.builder().setSpeech(speechBuilder.build());
   ...
   */
  @VisibleForTesting
  String getStreamName(int streamType) {
  ...
  private static class VolumeHandler extends WeakReferenceHandler<VolumeMonitor> {
    ...
    private final Duration releaseControlTimeoutMs;

    /** Timeout in milliseconds before the audio channel is available. */
    private static final Duration ACQUIRED_CONTROL_TIMEOUT_MS = Duration.ofMillis(2000);
    ...
    public VolumeHandler(VolumeMonitor parent) {
      super(parent);
      releaseControlTimeoutMs =
          FormFactorUtils.isAndroidWear()
              ? RELEASE_CONTROL_TIMEOUT_MS_WEAR
              : RELEASE_CONTROL_TIMEOUT_MS_DEFAULT;
    }
    ...
    public void handleMessage(Message msg, VolumeMonitor parent) {
      switch (msg.what) {
        ...
        case MSG_RELEASE_CONTROL -> parent.internalOnReleaseControl();
        ...
        case MSG_VOLUME_BUTTON_HINT -> parent.internalSpeakA11yVolumeControlHint();
        default -> {}
      }
    }
    ...
    public void releaseControlDelayed() {
      ...
      sendMessageDelayed(msg, releaseControlTimeoutMs.toMillis());
    }
    ...
    public void onControlAcquired(int type) {
      ...
      sendMessageDelayed(msg, ACQUIRED_CONTROL_TIMEOUT_MS.toMillis());
    }
     ...
     *
     * @param type the stream type
     * @param mutedState the current mute state
     */
